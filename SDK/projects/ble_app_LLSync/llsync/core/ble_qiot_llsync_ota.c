...
static ble_qiot_ret_status_t ble_ota_write_data_to_flash(void)
{
 ...
 void ble_ota_timer_callback(void *param)
{
...
#endif	
    ble_qiot_log_i("ble ota timer start ");
    ke_timer_set(APP_PERIOD_TIMER, TASK_APP, 200);	//timer 已经静态创建好，只需打开
    return;
}
static ble_qiot_ret_status_t ble_ota_init(void)
{
	...
	temp_p = (uint8_t *)&sg_ota_info;
    ble_qiot_log_hex(BLE_QIOT_LOG_LEVEL_INFO, "ota info", temp_p, sizeof(sg_ota_info));
