...
bool ble_is_connected(void)
{
 ...
 int memchk(const uint8_t *buf, int len)
{
    int i = 0;
...
ble_qiot_ret_status_t ble_init_flash_data(void)
{
	...
	ble_qiot_log_i("sg core data bind_state:%d",sg_core_data.bind_state);
...
#endif
	if (0 !=ble_get_device_info(&sg_device_info))
	{
		 ble_qiot_log_e("llsync get device info failed");
        return BLE_QIOT_RS_ERR_FLASH;
	}
...
int ble_get_my_broadcast_data(char *out_buf, int buf_len)
{
    ...
    if (E_LLSYNC_BIND_SUCC == llsync_bind_state_get()) {
        ...
        ble_qiot_log_hex(BLE_QIOT_LOG_LEVEL_INFO, "broadcast", sg_device_info.mac, BLE_QIOT_MAC_LEN);
