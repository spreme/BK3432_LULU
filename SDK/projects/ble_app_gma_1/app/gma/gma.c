...
void gma_ota_write_flash(void) 
{
    ...
    {
        //calccrc = genCrc16CCITT(calccrc, gma_ota_data.frame, gma_ota_data.frame_len);
...
void gma_reply_ota_fw_ver(uint8_t msg_id)
{
    ...
    data[1] = (ImgHdr.ver&0xff);//(uint8_t)((GMA_FIRMWARE_VER >> 0)  & 0xFF);   //firmware version
    data[2] = ((ImgHdr.ver>>8)&0xff);//(uint8_t)((GMA_FIRMWARE_VER >> 8)  & 0xFF);   //firmware version
    data[3] = (ImgHdr.rom_ver&0xff);//(uint8_t)((GMA_FIRMWARE_VER >> 16) & 0xFF);   //firmware version
    data[4] = ((ImgHdr.rom_ver>>8)&0xff);//(uint8_t)((GMA_FIRMWARE_VER >> 24) & 0xFF);   //firmware version
